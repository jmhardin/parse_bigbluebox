...
int main(int nvars, char** vars)
{
	...
	double dark_current = 12.5; //nA
	double min_wavelength = 314;
	if (nvars == 1 || (nvars == 4 && strcmp(vars[1],"-integrate") != 0) || nvars > 4)
	{
		printf("Usage: parse_reflectance <reference_file> <mirror_file>\n");
		printf("Outputs a 2 column list of wavelengths and reflectances.  Expects the two files to have the same number of rows.  Also has dark current hardcoded\n");
		printf("Usage: parse_reflectance -integrate <reference_file> <mirror_file>\n");
	...
	if (nvars == 4 && strcmp(vars[1],"-integrate") == 0)
	{
	...
	while (getline(ref_file,tmp_ref_line))
	{
		double tmp_wl = -1;
		double tmp_current = -1;
        	stringstream strstr(tmp_ref_line);
		strstr >> tmp_wl;
		strstr >> tmp_current;
		ref_wavelengths.push_back(tmp_wl);
		ref_currents.push_back(tmp_current);
    	}
	while (getline(data_file,tmp_data_line))
	{
		...
		data_currents.push_back(tmp_current);
    	}
